...
func NewRouter(deps *Dependencies) http.Handler {
	...
	mux.Handle("GET /v1/admin/keys/{id}/budget", applyAuthMiddleware(deps, HandleGetAPIKeyBudget(deps.DB)))
	mux.Handle("PUT /v1/admin/keys/{id}/budget", applyAuthMiddleware(deps, HandlePutAPIKeyBudget(deps.DB)))
	mux.Handle("DELETE /v1/admin/keys/{id}/budget", applyAuthMiddleware(deps, HandleDeleteAPIKeyBudget(deps.DB)))
	mux.Handle("POST /v1/admin/keys/{id}/budget/reset", applyAuthMiddleware(deps, HandleResetAPIKeyBudget(deps.DB)))
