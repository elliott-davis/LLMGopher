# Contract: Budget Management API

## Public Surface

This contract captures the externally visible behavior converted from `plans/07-admin-budget-api.md`. The exact endpoint, provider adapter, admin route, middleware behavior, storage record, metric, callback, or batch operation is defined by the original plan and refined by `spec.md`.

## Inputs

- Authenticated client, admin, provider, or operator input required to exercise Budget Management API.
...
- Existing gateway clients that do not opt into Budget Management API continue to work without behavior changes.
