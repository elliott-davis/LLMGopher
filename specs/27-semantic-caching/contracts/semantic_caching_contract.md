# Contract: Semantic Caching

## Public Surface

This contract captures the externally visible behavior converted from `plans/27-semantic-caching.md`. The exact endpoint, provider adapter, admin route, middleware behavior, storage record, metric, callback, or batch operation is defined by the original plan and refined by `spec.md`.

## Inputs

- Authenticated client, admin, provider, or operator input required to exercise Semantic Caching.
...
- Existing gateway clients that do not opt into Semantic Caching continue to work without behavior changes.
