# Implementation Plan: Prometheus Metrics Endpoint

**Branch**: `[20-prometheus-metrics]` | **Date**: 2026-04-26 | **Spec**: `spec.md`  
**Input**: Feature specification from `specs/20-prometheus-metrics/spec.md`

## Summary

Expose a /metrics endpoint in Prometheus text format covering request counts, latency, token usage, cost, and error rates. This provides a production-grade observability surface compatible with Grafana, alerting rules, and any Prometheus-compatible scraper. The original plan marks this feature pending; implementation and verification are both required.
...
**Scale/Scope**: Observability roadmap feature converted from `plans/20-prometheus-metrics.md`
...
```text
specs/20-prometheus-metrics/
├── spec.md
├── plan.md
├── research.md
├── quickstart.md
├── contracts/
│   └── prometheus_metrics_contract.md
└── checklists/
    └── requirements.md
```
...
go.opentelemetry.io/otel/metric is already an indirect dependency. However, Prometheus client is more direct and battle-tested for this use case. Add github.com/prometheus/client_golang directly. The existing slog logging in middleware and the cost worker provides the raw data; this spec instruments the same paths with metric counters/histograms.
