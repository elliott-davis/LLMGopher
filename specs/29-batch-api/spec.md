# Feature Specification: Batch API

**Feature Branch**: `[29-batch-api]`  
**Created**: 2026-04-26  
**Status**: Draft - pending implementation  
**Input**: Converted from `plans/29-batch-api.md`

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Submit Batch Jobs (Priority: P1)

API clients submit asynchronous batch workloads and retrieve results later.

**Why this priority**: Implement an OpenAI-compatible Batch API for asynchronous bulk processing of chat completion requests. Clients submit a batch of requests, receive a batch ID, poll for status, and retrieve results when complete. This enables high-throughput, cost-efficient processing of large workloads (evals, document processing, data enrichment).

**Independent Test**: Exercise the primary success path for Batch API through the gateway and verify the public behavior matches the converted acceptance criteria.

**Acceptance Scenarios**:

1. **Given** the gateway is configured for Batch API, **When** POST /v1/files with a valid JSONL file returns a file ID, **Then** the behavior is accepted for this feature.
2. **Given** the gateway is configured for Batch API, **When** POST /v1/batches creates a batch and transitions to in_progress, **Then** the behavior is accepted for this feature.
3. **Given** the gateway is configured for Batch API, **When** Batch worker processes all requests and writes output file, **Then** the behavior is accepted for this feature.
...
1. **Given** an invalid or unsupported request for Batch API, **When** the gateway rejects it, **Then** the response uses the expected public error contract.
...
Gateway operators can test, observe, and roll out Batch API without losing request traceability or spend accountability.
...
- **FR-001**: The system MUST deliver Batch API for the gateway behavior described in the converted roadmap plan.
...
- **FR-005**: The system MUST satisfy this acceptance behavior: POST /v1/files with a valid JSONL file returns a file ID.
- **FR-006**: The system MUST satisfy this acceptance behavior: POST /v1/batches creates a batch and transitions to in_progress.
- **FR-007**: The system MUST satisfy this acceptance behavior: Batch worker processes all requests and writes output file.
- **FR-008**: The system MUST satisfy this acceptance behavior: GET /v1/batches/{id} reflects accurate request_counts_completed.
- **FR-009**: The system MUST satisfy this acceptance behavior: GET /v1/files/{file_id}/content returns the output JSONL when batch is complete.
- **FR-010**: The system MUST satisfy this acceptance behavior: POST /v1/batches/{id}/cancel stops processing remaining requests.

### Key Entities *(include if feature involves data)*

- **Gateway Feature Configuration**: Runtime settings, database records, or provider metadata that enable Batch API.
...
- **SC-001**: The primary Batch API workflow succeeds through the gateway using the public API or admin API described by the converted plan.
...
- **CC-001**: Existing clients that do not use Batch API continue to work without request or response contract changes.
...
- Converted dependencies: Spec 01 (function calling) - batch requests may include tool calls; Spec 06 (audit log query) - batch results stored alongside regular audit entries.
- Out-of-scope boundaries from the original plan remain unchanged: Provider-native batch APIs (e.g., OpenAI's actual batch endpoint - this impl processes batches locally); Batch pricing discounts (cost is tracked at normal rates); Streaming batch results
